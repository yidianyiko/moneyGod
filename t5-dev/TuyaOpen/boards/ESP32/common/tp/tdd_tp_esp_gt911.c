/**
 * @file tdd_tp_esp_gt911.c
 * @brief One-shot init + register helper for GT911 capacitive touch panels via ESP-IDF esp_lcd_touch.
 *
...
/**
 * @brief Initialise a GT911 touch controller and register it as a TuyaOpen TDD tp device.
 * @param[in] name Device name used for later lookup.
 * @param[in] cfg  I2C / GPIO / TP configuration.
 * @return OPRT_OK on success, error code otherwise.
 */
OPERATE_RET tdd_tp_esp_i2c_gt911_register(char *name, TDD_TP_ESP_GT911_CFG_T *cfg)
{
    i2c_master_bus_handle_t       i2c_bus      = NULL;
    ...
    esp_lcd_touch_config_t tp_cfg = {
        ...
        .flags =
            {
                .swap_xy  = cfg->tp.tp_cfg.flags.swap_xy,
                .mirror_x = cfg->tp.tp_cfg.flags.mirror_x,
                .mirror_y = cfg->tp.tp_cfg.flags.mirror_y,
            },
    };
    ...
    esp_rt = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, &esp_tp_hdl);
    if (esp_rt != ESP_OK || NULL == esp_tp_hdl) {
        ESP_LOGE(TAG, "Failed to init gt911: %s", esp_err_to_name(esp_rt));
        return OPRT_COM_ERROR;
    }
