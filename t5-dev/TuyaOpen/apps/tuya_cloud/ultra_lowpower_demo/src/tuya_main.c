...
/* This is the dedicated ultra-low-power demo: ENABLE_WIFI_ULTRA_LOWPOWER is
 * always selected by its Kconfig (APP_CONFIG), so ULP code here is unconditional. */
...
/* deinit BLE stack after the device is online, to allow the SoC to reach deep
 * low-voltage sleep. On T5 the BT controller is powered by the CP core at boot
 * and only tuya_ble_deinit() -> tkl_hci_deinit() (an AP->CP IPC) powers it down. */
extern int tuya_ble_deinit(void);
#endif

/* Tuya device handle */
tuya_iot_client_t client;

/* If the device is not provisioned within this window, stop pairing (SoftAP/BLE)
 * and drop to low power - otherwise an unprovisioned device stays at full power
 * indefinitely (NETCFG is a permanent keep-awake lock). */
#define NETCFG_LOWPOWER_TIMEOUT_MS (3 * 60 * 1000) /* 3 min */
static TIMER_ID s_netcfg_lp_timer = NULL;

/**
 * @brief Netcfg timeout: if still unprovisioned, tear down pairing and let the
 *        SoC enter low power (mirrors the original's WWM_POWERDOWN handling,
 *        which TuyaOpen has no event for). The device then needs a reset/reboot
 *        to re-enter netcfg.
 */
static void netcfg_lowpower_timeout_cb(TIMER_ID timer_id, void *arg)
{
    if (tuya_iot_activated(&client)) {
        return; /* provisioned in time - nothing to do */
    }
    PR_NOTICE("ULP: netcfg timeout, stop pairing -> enter low power");
    netcfg_stop(NETCFG_STOP_ALL_CFG_MODULE); /* stop SoftAP + BLE netcfg */
#if defined(ENABLE_BLUETOOTH) && (ENABLE_BLUETOOTH == 1)
    tuya_ble_deinit(); /* power down the BT controller */
#endif
    lpmgr_unregister(TY_LP_NETCFG); /* release the full-awake netcfg locks */
...
void user_event_handler_on(tuya_iot_client_t *client, tuya_event_msg_t *event)
{
    ...
    case TUYA_EVENT_MQTT_CONNECTED: {
        /* Once online, tear down the BLE stack so the SoC can reach deep
         * low-voltage sleep - TuyaOpen's nimble stack holds a PM vote while
         * running and otherwise pins the idle floor at ~1.5mA. This is the
         * TuyaOpen adaptation of the original's "destroy BLE after netcfg":
         * here BLE is brought up early regardless of provisioning state and
         * ACTIVATE_SUCCESSED does not fire on reconnect, so MQTT-connected is
         * the reliable one-shot trigger. Re-pairing via BLE needs a reboot. */
        static bool s_ble_deinited = false;
        if (!s_ble_deinited) {
            s_ble_deinited = true;
            PR_NOTICE("ULP: online, deinit BLE to lower idle power");
            tuya_ble_deinit();
        }
    } break;
#endif
...
void user_main(void)
{
    ...
    /* ULP: long reconnect back-off (seconds) so a disconnected device keeps the
     * radio off longer between retries (grows to ~32min), mirroring the original
     * ULP reconnect tuning. Default netmgr table is only {1,3,5,10,15,20}. */
    static const uint32_t ulp_reconn_table[] = {1, 2, 3, 5, 10, 20, 30, 60, 120, 240, 480, 960, 1920};
    netmgr_conn_set(NETCONN_WIFI, NETCONN_CMD_RECONN_TABLE,
                    &(netmgr_reconn_table_t){.table = (uint32_t *)ulp_reconn_table,
                                             .size = sizeof(ulp_reconn_table) / sizeof(ulp_reconn_table[0])});
#endif
    ...
    tuya_wifi_ulp_init();
    lpmgr_set_lps_dtim(TY_LP_DITM_10);
    lpmgr_register(TY_LP_APP_USED);

    /* If unprovisioned, arm the netcfg-timeout guard: after the window, stop
     * pairing and drop to low power instead of staying at full power forever. */
    if (!tuya_iot_activated(&client)) {
        tal_sw_timer_create(netcfg_lowpower_timeout_cb, NULL, &s_netcfg_lp_timer);
        tal_sw_timer_start(s_netcfg_lp_timer, NETCFG_LOWPOWER_TIMEOUT_MS, TAL_TIMER_ONCE);
    }
