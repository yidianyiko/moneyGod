...
STATIC IPC_AUDIO_ENC_PARA_T *__ipc_para_alloc(IPC_AUDIO_ENC_OPS_E ops, TUYA_AI_ENCODER_INFO_T *info, INT_T out_buf_len)
{
    ...
    enc_para->ops = ops;
    enc_para->type = IPC_AUDIO_ENC_TYPE_SPEEX;
    if (info) {
...
STATIC OPERATE_RET _encoder_speex_ipc_create(AI_ENCODE_HANDLE_T *handle, TUYA_AI_ENCODER_INFO_T *info)
{
...
STATIC OPERATE_RET _encoder_speex_ipc_destroy(AI_ENCODE_HANDLE_T handle)
{
    OPERATE_RET rt = OPRT_OK;
...
STATIC OPERATE_RET _encoder_speex_ipc_encode(AI_ENCODE_HANDLE_T handle, UCHAR_T *in_buf, UINT_T in_len, AI_ENCODER_DATA_OUT_CB cb, void *usr_data)
{
    OPERATE_RET rt = OPRT_OK;
...
#define ENCODER_TIMESTAMP_PR 1
    ...
    TAL_PR_DEBUG("_encoder_speex_ipc_encode: start: %llu, end: %llu, delta: %llu(ms)", start, end, delta);
#endif
    ...
    if (cb && enc_para->out_buf && enc_para->out_len > 0) {
        rt = cb(AUDIO_CODEC_SPEEX, (UCHAR_T *)enc_para->out_buf, enc_para->out_len, usr_data);
        if (rt != OPRT_OK) {
            __ipc_para_free(enc_para);
            return rt;
        }
...
// Speex encoder throuth ipc
TUYA_AI_ENCODER_T g_tuya_ai_encoder_speex_ipc = {
    .handle = NULL,
    .name = "speex_ipc",
    .codec_type = AUDIO_CODEC_SPEEX,
    .create = _encoder_speex_ipc_create,
    .destroy = _encoder_speex_ipc_destroy,
    .encode = _encoder_speex_ipc_encode,
};
