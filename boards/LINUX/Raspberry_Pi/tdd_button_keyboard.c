...
typedef struct {
    ...
    char key_char;  // uppercase key character to match
    BUTTON_CFG_T cfg;
...
static OPERATE_RET __add_new_button(char *name, BUTTON_CFG_T *data, DEVICE_BUTTON_HANDLE *handle)
{
    ...
    // Store key_char in uppercase; fall back to name[0] if not set
    char kc = data->key_char ? data->key_char : name[0];
    p_ctx->key_char = (kc >= 'a' && kc <= 'z') ? kc - 'a' + 'A' : kc;
...
static OPERATE_RET __tdd_button_read_value(TDL_BUTTON_OPRT_INFO *dev, uint8_t *value)
{
    ...
    // Get the target key character (already stored in uppercase)
    name_upper = p_ctx->key_char;
