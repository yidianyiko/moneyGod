...
)
{
    ...
    for(;;) {               /* Parse JPEG segments */
        /* Get marker prefix + code (2 bytes) */
        if(jd->infunc(jd, seg, 2) != 2) return JDR_INP;
        ofs += 2;
        /* Skip fill bytes (0xFF) between markers per JPEG T.81 B.1.1.2 */
        while(seg[1] == 0xFF) {
            seg[0] = seg[1];
            if(jd->infunc(jd, &seg[1], 1) != 1) return JDR_INP;
            ofs++;
        }
        /* Get length field (2 bytes) */
        if(jd->infunc(jd, seg + 2, 2) != 2) return JDR_INP;
        ofs += 2;
        marker = LDB_WORD(seg);     /* Marker */
        len = LDB_WORD(seg + 2);    /* Length field */
        if(len <= 2 || (marker >> 8) != 0xFF) return JDR_FMT1;
        len -= 2;           /* Segent content size */
        ofs += len;         /* Number of bytes loaded */
